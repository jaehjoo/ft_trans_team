...
class PongTwoConsumers(AsyncWebsocketConsumer):
    ...
    async def join_matching(self):
        ...
        if room.status == "playing":
            await self.matchPlayers(room.players[0], room.players[1], room.players[2], room.players[3])
            await self.channel_layer.group_send(
                self.game_group_name, {
                    "type" : "game.message",
                    "data" : {
    ...
    def matchPlayers(self, player0, player1, player2, player3):
        ...
        self.player0, self.player0rating = sorted_players[0][0], sorted_players[0][1]
        self.player1, self.player1rating = sorted_players[1][0], sorted_players[1][1]
        self.player2, self.player2rating = sorted_players[2][0], sorted_players[2][1]
        self.player3, self.player3rating = sorted_players[3][0], sorted_players[3][1]
        self.save()
        ...
        with transaction.atomic():
            ...
            if is_room:
                ...
                if all(player != "" for player in is_room.players):
                    ...
                    setattr(self.RoomList, is_room.room_name, is_room)
